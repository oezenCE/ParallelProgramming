...
void Routine(void* data) {
	workData* dataS = (workData*) data;
	int start = dataS->start;
	int end = dataS->end;
	int height = dataS->height;
    int depth = dataS->depth;
	int samples = dataS->samples;
	auto spheres = dataS->spheres;
	int width = dataS->width;
	
	Camera camera = dataS->camera;
	int* image_data = dataS->image_data;
	Checksum* checksum = dataS->checksum;
...
int main(int argc, char **argv) {
    ...
    int no_output = 0;
    char file_name[256] = "rendersie.ppm";
    int c;
    ...
    int noThread = 32;
    std::vector<std::thread> threadPool(noThread);
    std::vector<workData*> workPool(noThread);
    int capacity = height/noThread + 1; 
   

    


    for(int thread = 0; thread<noThread; thread++){
        int* ptr = new int(thread);
        
        int start = height - 1 - capacity * (*ptr);
		int end = std::max(start - capacity, -1);
		workPool[*ptr] = (workData*) malloc(sizeof(workData));
		workData* data = workPool[*ptr];
		data->start = start;
		data->end = end;
        data->depth = depth;
        ...
        threadPool[*ptr] = std::thread(Routine, data);
