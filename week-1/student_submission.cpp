...
void substitute_bytes() {
    ...
    int dic[UNIQUE_CHARACTERS];

    for (int i=0; i<UNIQUE_CHARACTERS; i++){
        uint8_t oKey = originalCharacter[i];
        dic[oKey] = substitutedCharacter[i];
    }
 ...
 for (int column = 0; column < MESSAGE_BLOCK_SIZE; column++) {
        for (int row = 0; row < MESSAGE_BLOCK_SIZE; row++) {
            ...
            // and replace it with corresponding the element in the substituted character list
                 message[row][column] = dic[message[row][column]];
 ...
 *
 */
//uint8_t powers[MESSAGE_BLOCK_SIZE][MESSAGE_BLOCK_SIZE];
...
/*void pow (){
    for (int x=0; x< MESSAGE_BLOCK_SIZE; x++){
        for (int y=0; y< MESSAGE_BLOCK_SIZE; y++){
            powers[x][y]=power(x, y+1);
        }
    }
}
*/
